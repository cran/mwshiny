...
#' @param serv_calc a named list of functions that calculate variables derived from user input, to be used in rendering output. Each function is of the form function(calc), where calc is a named list containing the traditional shiny input and user-created reactive values. All calculated variables that are needed to render output should be added, named, to the calc list. When using reactive functions such as observeEvent(), each should be contained in a separate function, and variables dependent on these reactions should be added to calc. Note that these functions follow all shiny conventions (reactive values must be accessed in a reactive context, etc.).
#' @param serv_out a named list of functions that render output. Each function is of the form function(calc), where calc is a named list containing the traditional shiny input and reactive values that have calculated values derived from input. It returns the results of a shiny render function. The name of each function corresponds to its output label. Note that these functions follow all shiny conventions (reactive values must be accessed in a reactive context, etc.).
...
#' serv_out <- list()
#' serv_out[["clickplot"]] <- function(calc){
#'   renderPlot({
...
mwsApp <- function(win_titles=c(), ui_win=list(), serv_calc=list(), serv_out=list(), depend = list()){
  ...
  mws_server <- shiny::shinyServer(function(input,output){
      ...
      for (s in 1:length(serv_calc)){
        ...
        tryCatch({
          ...
          if (length(grep("unused argument ", as.character(e)[1], fixed = T))>0 |
            ...
            e$message <- paste("Argument", s, "of serv_calc's functions does not have 1 argument")
          }
          stop(e)
        })
      }
    ...
    serverFunct(serverValues, output, serv_out)
